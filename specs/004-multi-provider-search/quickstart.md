...
- 迅雷字幕 provider：当前无需凭据；`v0.2.2` 走「不依赖数据库 schema 的最小接入路径」，迅雷 provider 元数据由 `provider-registry.ts` 代码层硬编码，**不**需要在 `providers` 表中插入 `type=xunlei` 记录

### 1.4 数据库 schema（`v0.2.2` 不变更）

> ⚠️ **范围声明**：本次 `v0.2.2` **不**做任何数据库 schema 变更。不需要执行 `pnpm db:generate` / `pnpm db:migrate` 以扩展 `providerTypes` enum。

如后续需要执行 `pnpm db:check`，仅验证现有 schema 健康，**不应**包含本次新增的 enum 扩展。如发现 `drizzle/` 目录在本次 PR 出现新 migration 文件，应拒绝合并。
...
### 2.2 迅雷 Provider（`v0.2.2` 走代码层硬编码，**不**入库）

> ⚠️ **范围声明**：本次 `v0.2.2` 不修改 `providerTypes` enum，**不**在 `providers` 表中插入 `type=xunlei` 记录。迅雷 provider key 由 `src/server/providers/provider-registry.ts` 在代码层硬编码，gateway 通过该注册表调度。

- **不需要**执行 `INSERT INTO providers` 等 SQL 操作。
- **不需要**修改 `scripts/db/seed-dev.ts` 或 `scripts/db/seed-staging.ts` 追加迅雷 provider 种子。
- 迅雷 provider 在 `v0.2.2` 的启用 / 禁用 / 限流 / 冷却等调度控制，仅由代码层 feature flag / 环境变量决定（如 `XUNLEI_PROVIDER_ENABLED`）。
...
- 在代码层通过环境变量禁用迅雷 provider（如 `XUNLEI_PROVIDER_ENABLED=false`，具体以实现为准），或
- 通过 mock fetch 实现构造上游 5xx 响应（在 adapter 单元测试中），或
- 在 adapter 入口注入受控的失败（如临时改 adapter 实现返回 `upstream_failed`）

> 由于 `v0.2.2` 不在 `providers` / `provider_credentials` 表中持久化迅雷 provider，本场景**不**使用 `UPDATE provider_credentials ... WHERE provider_id = 'provider_xunlei_default'` SQL；该 provider 不存在于 `providers` 表。
...
- 响应 `data.provider_failures[]` 含 `provider === "xunlei"` 与 `reason`（如 `upstream_failed`）
- 响应 `data.status === "partial"`（因为至少一个 provider 失败）

**恢复**（测试完成后）：恢复环境变量 / mock 状态。
...
- OpenSubtitles：在数据库中将 OpenSubtitles provider 的所有凭据 `status` 设为 `disabled`：

```sql
UPDATE provider_credentials SET status = 'disabled' WHERE provider_id = 'provider_opensubtitles_default';
```

- 迅雷 provider：在代码层通过环境变量禁用（如 `XUNLEI_PROVIDER_ENABLED=false`）。

> 注意：本次 `v0.2.2` 迅雷 provider 不在 `providers` / `provider_credentials` 表中持久化，**不**应执行无 `WHERE provider_id = 'provider_xunlei_default'` 的 SQL。
...
```sql
UPDATE provider_credentials SET status = 'active' WHERE provider_id = 'provider_opensubtitles_default';
```

并恢复迅雷 provider 的环境变量。
