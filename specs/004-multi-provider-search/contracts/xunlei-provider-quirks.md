...
| `name` + `languages`                                                                                                                 | `language`                                                  | 用 `name`（优先）+ `languages[0]`（兜底）归一化为 `zh-CN`/`zh-TW`/`en`/`zh-CN,en`/`zh-TW,en` 或 `es`/`ko`/`ja`/`fr`/`de`/`pt`/`ru`/`it`/`th`/`vi`/`ar`（无法判定为 `null`）；`releaseName` 中的中文语言标记（`西语`/`韩文`/`英文`/`中文` 等）与拉丁标记（全称/短码）都会被提取；原始值保留在 `raw.languages`                                                                                                                                                                                                                                                                                                                      |
...
- 字段消费：传入 `query + language`，验证上游 URL 只含 `name`、不含 `languages`
- 字段忽略：传入 `imdb_id` 等不消费字段，验证上游 URL 不包含
- 必要条件缺失（`query` 空）：返回 `skipped: true, reason: 'missing_required_field'`
- 语言归一化：`language` 由 `name` + `languages[0]` 归一化（`CHSEN`/中英 → 双语，空语言 + 中文名 → `zh-CN`，`西语`/`韩文`/`英文` 等标记 → `es`/`ko`/`en` 等真实语言码）
