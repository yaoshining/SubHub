...
- 当前端实现涉及页面图标、导航图标、状态图标、动作图标时，默认优先使用 `lucide-react`。
- 若设计稿、page spec 或实现说明已给出 Lucide 图标名，优先使用同名 `lucide-react` 组件，不自行替换近似图标。
- 若设计稿与代码中的图标命名不一致，必须显式指出并收敛命名，不得静默偏离。
- 同一语义在同一产品域内不应随意替换为不同图标。
- 仅当 Lucide 中不存在合适图标时，才提示需要额外图标源，并先说明原因，不直接改用其他图标库。
