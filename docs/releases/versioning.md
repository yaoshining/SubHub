...
- 当前仓库已建立版本 milestones：`v0.1.0`、`v0.2.0`、`v0.2.1`、`v0.2.2`、`v0.3.0`、`v1.0.0`。
...
- 当 issue 属于当前首发闭环，应优先归入 `v0.1.0`；数据库与部署生产化工作应优先归入 `v0.2.0`；已发布 API 的非 breaking 字段扩展应优先归入 `v0.2.x`（如 `v0.2.1`）；多 provider 搜索入口模型工作应优先归入 `v0.2.2`；字幕资产管理基础版工作应优先归入 `v0.3.0`；正式稳定首发收口工作应优先归入 `v1.0.0`。
