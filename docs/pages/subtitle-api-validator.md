...
- Provider 声明为搜索必填的字段必须标记“必填”；提交前应以字段级错误列出缺失项并阻止请求，服务端绕过 UI 的请求也必须返回相同的明确参数错误。
- Xunlei 的“附加查询”是必填并映射为上游 `name`；“语言”是可选筛选，映射为上游 `languages`。语言控件必须同时提供已知值下拉（`简体`、`默认`、`英语`）与自定义输入，留空时不得传递 `languages`；自定义值仅用于传递迅雷上游的原生语言标签，不得假定其为 BCP-47 语言码。
