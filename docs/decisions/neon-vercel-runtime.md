...
**关联 issue**：`#62`（002 主追踪 issue）、`#64`（production runtime readiness）、`#66`（Vercel / GitHub Actions migration & deploy gate 收尾）、`#70`（最小 migration / deploy gate 边界）、`#68`（本文档所属 issue：002 回写迁移决策、runbook 与 quickstart 收尾）。
...
## 11. v0.2.0 收口与 SQLite / Postgres 边界确认

本文档对应的实现版本为 `v0.2.0`；到该版本收敛时，SubHub 的运行基线状态如下：

1. **正式运行基线**：Neon Postgres + Vercel。
2. **SQLite 定位**：已降级为 `001-mvp-admin-console` 阶段的历史实现参考，不再作为当前正式交付中的部署目标或数据迁移来源。
3. **Postgres 定位**：成为 SubHub 当前 MVP 的正式运行基线；Postgres schema baseline 独立建立，SQLite 时代的 migration 历史不做 1:1 继承。
4. **数据搬迁范围**：SQLite 历史数据迁入 Neon 不属于 `v0.2.0` 的正式交付要求；`v0.2.0` 不再以"先完成 SQLite 历史数据迁移"为上线前置条件。
5. **运行边界**：`v0.2.0` 不引入自动 rollback、不引入多阶段自动 promotion pipeline、不引入平台级长期发布治理框架；这些能力如后续需要，应作为 `v0.2.x` / future 扩展单独建模。
6. **真源关系**：
   - 仓库级运行时环境映射与 Preview 分支白名单真源：`docs/runtime/environment-mapping.md`
   - 运行手册与操作步骤：`docs/workflows/vercel-neon-environments.md`
   - 002 feature spec / plan / tasks：`specs/002-migrate-neon-vercel/`
   - 001 数据库落地设计（保留 001 数据语义不变）：`specs/001-mvp-admin-console/database-design.md`
