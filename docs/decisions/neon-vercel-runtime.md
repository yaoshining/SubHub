...
| Development | 其他 `preview/*`、`agent/*`、`feature/*` 等非 `main`、非 `preview` 分支 | Vercel Preview 或本地开发 | dev 数据库 |
...
- 其他 `preview/*`、`agent/*`、`feature/*` 分支默认共享 dev 数据库。
...
5. 后续实现需要单独定义 SQLite 到 Postgres 的迁移与 cutover 方案，但该方案属于实现/部署 spec，不在本文展开。
...
- SQLite 现有数据迁移到 Postgres 的 cutover 步骤与回滚原则
