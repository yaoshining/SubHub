...
describeWhenLocalPostgresEnabled(
  ...
  () => {
    ...
    it("rotates caller keys transactionally on the real Postgres path", async () => {
      ...
      const persistedRotations = await directDb
        ?.select()
    ...
    it("keeps 24h usage counts and recent-item limits on the real Postgres path", async () => {
      ...
      await Promise.all(
        Array.from({ length: 21 }, (_, index) =>
          repository.recordSearchRequest({
            ...
            durationMs: 120,
            createdAt: new Date(
              now.getTime() + 61_000 + index * 60_000,
            ).toISOString(),
          }),
        ),
      );
      await Promise.all(
        Array.from({ length: 21 }, (_, index) =>
          repository.recordDownloadRequest({
            ...
            durationMs: 80,
            createdAt: new Date(
              now.getTime() + 62_000 + index * 60_000,
            ).toISOString(),
          }),
        ),
      );
      ...
      const usageSummary = await repository.getUsageSummary(
        created.callerKey.id,
        now,
      );
      const persistedSearches = await directDb
        ?.select()
        .from(subtitleSearchRequests);
      const persistedDownloads = await directDb
        ?.select()
