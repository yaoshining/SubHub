...
describeWhenLocalPostgresEnabled("ProviderRepository on local Docker Postgres", () => {
  ...
  it("persists encrypted provider credentials and returns sanitized detail on the real Postgres path", async () => {
    ...
    const persistedCredentials = await directDb?.select().from(providerCredentials);
    ...
    expect(persistedCredentials?.[0]?.secretEncrypted).toEqual(expect.any(String));
  ...
  it("enforces per-provider credential uniqueness on the real Postgres path", async () => {
    ...
    expect(persistedCredentials?.map((credential) => credential.label).sort()).toEqual([
      "primary",
      "secondary",
    ]);
  });
